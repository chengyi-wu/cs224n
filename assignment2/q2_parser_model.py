...
class ParserModel(Model):
    ...
    def add_prediction_op(self):
        ...
        W = tf.Variable(xavier_initializer(shape=(config.n_features * config.embed_size, config.hidden_size)))
        b1 = tf.Variable(tf.zeros(config.hidden_size))
        U = tf.Variable(xavier_initializer(shape=(config.hidden_size, config.n_classes)))
