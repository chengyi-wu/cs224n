...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        # pass
        new_sentence = [zero_vector] * max_length
        new_labels = [zero_label] * max_length
        _masking_ = [False] * max_length
        for i in range(min(len(labels), max_length)):
            new_sentence[i] = sentence[i]
            new_labels[i] = labels[i]
            _masking_[i] = True
        ret.append((new_sentence, new_labels, _masking_))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        h0 = tf.zeros(
            shape=(tf.shape(x)[0], Config.hidden_size)
        )
        h_t = h0
        ### END YOUR CODE
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                x_t = x[:, time_step]
                o_t, h_t = cell(x_t, h_t)
                tf.get_variable_scope().reuse_variables()
                o_drop_t = tf.nn.dropout(o_t, self.dropout_placeholder)
                preds_t = tf.matmul(o_drop_t, U) + b2
