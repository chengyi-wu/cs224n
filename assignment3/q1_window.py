...
class Config:
    ...
    # 1 = current word, 2 * window_size is the context words
    n_window_features = (1 + 2 * window_size) * n_word_features 
    ### END YOUR CODE
    n_classes = 5
    dropout = 0.5
    embed_size = 50
...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        for i, lbl in enumerate(labels):
            data_point = []
            for idx in range(i - window_size, i + window_size + 1):
                if idx < 0:
                    data_point.extend(start)
                elif idx > len(sentence) - 1:
                    data_point.extend(end)
                else:
                    data_point.extend(sentence[idx])
            windowed_data.append((data_point, lbl))
    ### END YOUR CODE
